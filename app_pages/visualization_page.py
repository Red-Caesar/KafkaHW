...
def visualization_page(processor):
        ...
        if symbol:
            ...
            if data and data["symbol"] == symbol:
                ...
                if metrics and "metrics" in metrics:
                    ...
                    with col1:
                        st.metric(
                            "Current Price", f"${price_metrics['current_price']:.2f}"
                        )
                    with col2:
                        st.metric(
                            "Price Change",
                    ...
                    with col3:
                        st.metric(
                            "Highest Price", f"${price_metrics['highest_price']:.2f}"
                        )
                    with col4:
                        st.metric(
                    ...
                    with col2:
                        fig_returns = go.Figure(
                            data=[
                                go.Histogram(
                                    x=metrics["technical_indicators"]["Daily_Return"],
                                    nbinsx=30,
                                    name="Daily Returns",
                                )
                            ]
                    ...
                    if all(pd.notna(v) for v in fin_metrics.values()):
                        ...
                        with fin_col2:
                            ...
                            st.metric(
                                "Revenue",
                                f"${revenue/1e9:.1f}B" if pd.notna(revenue) else "N/A",
                            )
                        with fin_col3:
                            roe = fin_metrics["ROE"]
                            st.metric(
